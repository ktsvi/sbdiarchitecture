...
import urifia.gaml.architecture.sbdi.*;
...
@facets (
		value = { @facet (
				...
				@facet (
						name = PerceiveStatement.SPATIAL_KNOWLEDGE,
						type = SpatialKnowledgeType.id,
						optional = true,
						doc = @doc ("The spatial_knowledge needed to do the perception")),
				@facet (
...
public class PerceiveStatement extends AbstractStatementSequence {
	...
	public PerceiveStatement(final IDescription desc) {
		...
		spatial_knowledge = getFacet(PerceiveStatement.SPATIAL_KNOWLEDGE);
		sk_power = getFacet(PerceiveStatement.SK_POWER);
	...
	public Object privateExecuteIn(final IScope scope) throws GamaRuntimeException {
		...
		if (_when == null || Cast.asBool(scope, _when.value(scope))) {
			...
			if (spatial_knowledge == null || SbdiArchitecture.hasSpatialKnowledge(scope, (SpatialKnowledge) spatial_knowledge.value(scope))) {
				if (sk_power == null || spatial_knowledge != null && sk_power != null && SbdiArchitecture.getSpatialKnowledge(scope,
						(SpatialKnowledge) spatial_knowledge.value(scope)).sk_power >= (Double) sk_power.value(scope)) {
